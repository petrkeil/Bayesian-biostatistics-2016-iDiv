...
This can also be written as $$\mu =\! \int_0^\infty S(u)du$$ which can be easier to deal with (the proof is [here](http://thirdorderscientist.org/homoclinic-orbit/2013/6/25/the-darth-vader-rule-mdash-or-computing-expectations-using-survival-functions)).
...
```{r}
  seedlings <- read.table("http://www.petrkeil.com/wp-content/uploads/2016/01/seedlings.txt", header=TRUE)
```
...
The `status` variable indicates if the observation is censored (0) or if it represents real time to death (1).
